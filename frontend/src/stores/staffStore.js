...
const useStaffStore = create((set, get) => ({
  ...
  fetchOrderDetails: async (orderId) => {
    set({ loading: true, error: null });
    try {
      const response = await axios.get(`${API_BASE_URL}/staff/orders/${orderId}/details`);
      if (response.data.success) {
        set({ loading: false });
  ...
  updateOrderStatus: async (orderId, status, notes = '') => {
    set({ loading: true, error: null });
    try {
      const response = await axios.put(`${API_BASE_URL}/staff/orders/${orderId}/status`, {
        status,
        notes
      });
      if (response.data.success) {
  ...
  fetchAlerts: async () => {
    set({ loading: true, error: null });
    try {
      const response = await axios.get(`${API_BASE_URL}/staff/alerts`);
      if (response.data.success) {
        set({
  ...
  startTimer: async (orderId) => {
    try {
      const response = await axios.post(`${API_BASE_URL}/staff/start-timer`, {
        orderId
      });
      if (response.data.success) {
